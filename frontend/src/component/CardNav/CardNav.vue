...
<template>
  <div class="card-nav-container" :class="props.className">
    <nav
      ref="navRef"
      :class="[
    ...
    >
      ...
      >
        <div
          v-for="(item, idx) in (props.items || []).slice(0, 4)"
          :key="`${item.label}-${idx}`"
          :ref="setCardRef(idx)"
        ...
        >
          ...
          <div class="flex flex-col gap-[2px] mt-auto nav-card-links">
            <router-link
              ...
              :to="lnk.href && lnk.href.startsWith('/') ? lnk.href : '/'"
